...
<Tip>You can find your Pulse API key in your Prisma Postgres connection string, it's the value of the `api_key` argument and starts with `ey...`. Alternatively, you can find the API key in your [Prisma Postgres Dashboard](https://console.prisma.io).</Tip>

Then, update the `env` package to include the new `PULSE_API_KEY` environment variable:

```ts packages/env/index.ts
export const server = {
  // ...
