...
Environment variables are defined in `.env.local` and validated in `packages/env/index.ts`. The file exports an `env` object that contains all validated environment variables, separated into `server` and `client` variables.

## Initial setup

### App Environment Variables

As part of the initial setup, you will need to fill in the environment variables in the `.env.local` file in the root of the project. This file will be symlinked to the `.env` file in each Next.js application, specifically `app`, `web` and `api`; as well as the `database` package.
