...
The monorepo is managed by [Turborepo](https://turborepo.com/repo), which is a tool for managing monorepos. It provides a simple way to manage multiple packages in a single repository, and is designed to work with modern web applications.
...
next-forge uses [Turborepo's boundaries](https://turborepo.com/repo/docs/reference/boundaries) to ensure that Turborepo features work correctly by checking for package manager Workspace violations.
