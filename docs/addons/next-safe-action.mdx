...
.schema(
  ...
  .action(async ({ parsedInput: { name, id } }) => {
    ...
    return data // Return here the value to the client
  });
```
...
In this example, we create an action with input validation on the server, and call it on the client to with typesafe inputs and convinient callback utilities to simplify state management and error handling.
