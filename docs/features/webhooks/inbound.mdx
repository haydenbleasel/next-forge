...
[Payment events](/features/payments) are handled in the `POST /webhooks/stripe` route in the `api` app. This route constructs the event and then switches on the event type to determine how to process the event.
...
[Authentication events](/features/authentication) are handled in the `POST /webhooks/clerk` route in the `api` app. 
