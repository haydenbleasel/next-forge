...
Generate a secret with the following command to add it to the `.env.local` file in each package:
...
Replace the contents of the `server.ts` file in the `auth` package with the following code:
