...
next-forge uses [Vitest](https://vitest.dev/) for unit testing. It's a fast and lightweight testing framework that's compatible with Jest's API.
...
Make sure you replace `[app]` with the name of the app you're adding Vitest to. Then, create a new file in the `__tests__` folder in the relevant app and add a `test` command to the `package.json` file:
...
Turbopack will automatically pick up on the new `test` script and run the tests.
