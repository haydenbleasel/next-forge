...
```ts title="apps/app/app/(authenticated)/page.tsx"
import { database } from '@repo/database';
import { page } from '@repo/database/schema'; // [!code ++]

// ...

const App = async (): Promise<ReactElement> => {
  const pages = await database.page.findMany(); // [!code --]
  const pages = await database.select().from(page); // [!code ++]
